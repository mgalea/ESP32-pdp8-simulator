...
esp_ip4_addr_t ip_Addr;				 // IP address of STA
...
enum
{
	...
	WAIT_ANY,
	WAIT_CMD
} menu_state = START; // State of menu
		...
		for (blk = 0; blk < size; blk++) // Loop for all blocks from device
		{
			...
			if (istape)					   // Is it DECtape
			{
				n = fwrite(wrdbuf, sizeof(uint16_t), // Write 128 words into DECtape block
					...
					fwrite(&dumwrd, sizeof(uint16_t), // Write a dummy 129th word
						   1, f) -
					2; // Correction for 2 dummy words
			}
			else
			{
	...
	for (bcnt = 0; bcnt < size; bcnt++) // Copy whole file or half of file
	{
		...
		if (istape)
		{
			n = fread(wrdbuf,
				...
				fread(&dumwrd,
					  sizeof(uint16_t), 1, f) -
				2; // Read unused word, compensate for dummies
		}
		else
		{
...
void menu()
{
	const char *menutxt =
		...
		"BO - Boot from RKA0:"
		"\e[8;0f"
		"ST - Start OS/8 at 0:7605"
		"\e[9;0f"
		"UC - Toggle UPPER case flag"
		"\e[10;0f"
		"SR - Set switch register"
		"\e[11;0f"
		"PR - Set filename for PTR"
		"\e[12;0f"
		"UP - Software update (OTA)"
		"\e[5;40f"
		"PO - Power-off"
		"\e[6;40f"
		"DW - Download RKA0+RKB0 image"
		"\e[7;40f"
		"SL - Select device for LD/SV"
		"\e[8;40f"
		"LD - Load image from SD card"
		"\e[9;40f"
		"SV - Save image to SD card"
		"\e[10;40f"
		"FL - Flush cache buffers"
		"\e[11;40f"
		"PP - Set filename for PTP"
		"\e[12;40f"
		"BF - Back to factory version"
		"\e[14;0f"
		"Option: ";
...
static void event_handler(void *ctx, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
	...
	if (event_base == IP_EVENT)
	{
		...
		case SYSTEM_EVENT_STA_GOT_IP:
			...
			esp_netif_get_ip_info(IP_EVENT_STA_GOT_IP, &ip_info);	 // Get IP address of station
			MQ = ip_info.ip >> 24;					   // Show LS digit of IP in MQ
...
static void initialize_wifi(void)
{
	...
	if (strlen((const char *)wifi_config.sta.ssid) < 1 || strlen((const char *)wifi_config.sta.password) < 1)
	{

		ESP_LOGI(tag, "wifi_init_sta not initialized.");
		...
		ESP_LOGI(tag, "SETTING PASSWORD TO %s", CONFIG_WIFI_SSID);
	}
	else
	{
		ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));
		ESP_LOGI(tag, "wifi_init_sta is initialized.");
	}

	esp_netif_set_hostname(TCPIP_ADAPTER_IF_STA, "ESP32_PDP8");
...
void opr()
{
	if ((IR & 0400) == 0) // group 1
	{
		...
		case 04:									// RAL
			...
			break;
		case 02: // BSW
			AC = (AC & 010000) | ((AC >> 6) & 077) | ((AC << 6) & 07700);
			break;
		}
	}
...
void app_main()
{
	...
	// ESP_LOGI(tag, "Booted from %s partition", esp_ota_get_boot_partition()->label);
